...
class PDF extends FPDF
{
    ...
    function Data()
    {
       ...
       /* $this->SetFont('Arial', 'B', 10);
        ...
        $this->Image('images/codigos.png', 10, 165, 190);
    ...
    function DescripcionServicio($servicio1, $servicio2, $servicio3, $servicio4, $servicio5, $servicio6, $abono, $abono2, $iva, $subtotal, $ahPagado, $aPagar)
    {
        ...
        $this->SetFillColor(255, 255, 0);
        $this->SetTextColor(0);
        $this->Cell(94, 7, utf8_decode('Costo del servicio'), 0, 0, 'C', true);
        $this->Ln();
        ...
        $this->SetFillColor(255, 255, 0);
        $this->SetTextColor(0);
        $this->Cell(47, 5, utf8_decode('Anticipo'), 1, 0, 'C', true);
        $this->Cell(47, 5, utf8_decode('A pagar día de la carga'), 1, 0, 'C', true);
        $this->Cell(47, 5, utf8_decode('A pagar antes de la descarga'), 1, 0, 'C', true);
        $this->Ln();
        $this->SetFillColor(255);
        $this->Cell(53);
        $resultado =  sprintf("%.2f", ($aPagar - $abono - $abono2));
        if ($ahPagado === 'Pendiente') {
            $this->SetFillColor(255);
            $this->Cell(47, 5, '$' . sprintf("%.2f", $abono), 1, 0, 'C', true);
            $this->Cell(47, 5, '$' . sprintf("%.2f", $abono2), 1, 0, 'C', true);
            $this->Cell(47, 5, '$' . $resultado, 1, 0, 'C', true);
        } else {
            $this->Cell(47, 5, '$' . sprintf("%.2f", $aPagar), 1, 0, 'C', true);
            $this->Cell(47, 5, '$' . sprintf("%.2f", '0'), 1, 0, 'C', true);
            $this->Cell(47, 5, '$' . sprintf("%.2f", '0'), 1, 0, 'C', true);
        }
        $this->Ln(18);
        //Firmas
    ...
    function HeaderMudanzas($creado)
    {
        ...
        $this->Cell(73.5);
                ...
                $ExecuteSelectDataInfo = mysqli_query($conexion, $SelectDataInfo);
                $rowInfo = mysqli_fetch_assoc($ExecuteSelectDataInfo);
                if($rowInfo["TipoServicio"] == "Traslado Vehiculo" || $rowInfo["TipoServicio"] == "Movimientos Especiales"){
                    $titulo = "Orden de Servicio - ".$rowInfo["TipoServicio"];
                }else{
                    $titulo = "Carta Porte";
                }
        $this->SetX(60);
        $this->Cell(30, 17, utf8_decode($titulo));
        $this->Cell(51);
        $this->SetFillColor(0, 0, 0);
        $this->SetTextColor(255, 255, 255);
        $this->SetDrawColor(0, 0, 0);
    ...
    function Pagina2()
    {
        $this->Image('images/Porte_G_1.png', 15, 20, 180, 235);
    }

    function Pagina3()
    {
        $this->Image('images/Porte_G_2.png', 15, 20, 180, 235);
    }

    function Pagina4()
    {
        $this->Image('images/Porte_G_3.png', 15, 20, 180, 235);
    }

    function Pagina5()
    {
        $this->Image('images/Porte_G_4.png', 15, 20, 180, 235);
    }

    function Pagina6()
    {
        $this->Image('images/Porte_G_5.png', 15, 20, 180, 235);
    }

    function Pagina7()
    {
        $this->Image('images/Porte_G_6.png', 15, 20, 180, 235);
    }
}
